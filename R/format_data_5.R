...
time <- gsub(":", "-", time)
#Save data frame with raw values from each file.
write.csv(df, file=paste0(file_path,"/counts/",time,"_DDX4_EdU_nuclei.csv"), row.names = FALSE)
...
if(length(filenames) > 1){
  ...
  time <- gsub(":", "-", time)
  #Save data frame with raw values from each file.
  write.csv(df, file=paste0(file_path,"/counts/",time,"_DDX4_EdU_cytoplasm.csv"), row.names = FALSE)
}
...
if(length(filenames) > 1){
  ...
  time <- gsub(":", "-", time)
  #Save data frame with raw values from each file.
  write.csv(df, file=paste0(file_path,"/counts/",time,"_DDX4_EdU_secondaryobjects.csv"), row.names = FALSE)
}

#Get file names for all files in the folder (including subfolders) indicated by "path" that are the file type indicated by "pattern".
#This pattern gets the files with object relationships.
filenames <- list.files(path=file_path, pattern = "*_env.csv", full.names=TRUE, recursive = TRUE)
if(length(filenames) > 1){
  #Load all files as data frames in the list of data frames "ldf".
  ldf <- lapply(filenames, read.csv)
  #Create data frame to store all results in.
  ...
  time <- gsub(":", "-", time)
  #Save data frame with raw values from each file.
  write.csv(df, file=paste0(file_path,"/counts/",time,"_DDX4_EdU_env.csv"), row.names = FALSE)
}
...
for (i in 1:length(per_plate)){
  ...
  file <- paste0(file_path,"/counts/", u.Plate[i,1],"/",u.Plate[i,1],"_per_image_DDX4EdUv7_quantification.xlsx")
  save <- per_plate[[i]]
...
for (i in 1:length(per_plate)){
  ...
  file <- paste0(file_path,"/counts/",u.Plate[i,1], "/",u.Plate[i,1],"_per_well_DDX4EdUv7_quantification.xlsx")
  save <- per_plate[[i]]
  write.xlsx2(save,file)
}
...
lapply(1:length(per_plate), function(a) write.xlsx2(per_plate[[a]], 
                                                    file = paste0(file_path, "/counts/", u.Plate[a,1], "/",filename[a,1]),
                                                    row.names = FALSE))
