...
for (i in 1:length(per_plate)){
  ...
  file <- paste0(file_path,"/counts/",u.Plate[i,1], "/",u.Plate[i,1],"_DDX4EdUv7_quantification.xlsx")
  save <- per_plate[[i]]
  write.xlsx(save,file)
}
...
for (i in 1:length(plate_summary)){
  write.xlsx(plate_summary[[i]], paste0(file_path,"/counts/", u.Plate[i,1],"/",filename[i,1]))
}